...
use MVC\ImageContainer;
use MVC\ImageManager;
use MVC\Router;
...
class PageController
{
  public static function index(Router $router): void
  {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
      $format = $_POST['format'];
      // showFormat($_FILES, true);
      $image = new ImageManager(new ImageContainer, $format);
      $image->setFiles($_FILES);
      $image->make();
      sleep(1);
      ImageContainer::scanFile($image->imageName, $format);
    }
